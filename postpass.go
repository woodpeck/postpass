...
const (
    ...
    password = "readonly"
    dbname   = "gis"
    quick_medium_threshold = 100
    medium_slow_threshold = 50000
    listen_port = 8081
   )

// encapsulates a response generated by a worker
type SqlResponse struct {
    err bool
    result string
}

// encapsulates a job sent to a worker
...
func handleApi(db *sql.DB, slow chan<- WorkItem, medium chan<- WorkItem, quick chan<- WorkItem, writer http.ResponseWriter, r *http.Request) {
    ...
    geojson := true;
    t_geojson := r.Form["options[geojson]"]
    if (t_geojson != nil) { geojson, _ = strconv.ParseBool(t_geojson[0]) }
    
    pretty := true;
    t_pretty := r.Form["options[pretty]"]
    if (t_pretty != nil) { pretty, _ = strconv.ParseBool(t_pretty[0]) }
    
    collection := true;
    t_collection := r.Form["options[collection]"]
    if (t_collection != nil) { collection, _ = strconv.ParseBool(t_collection[0]) }
    ...
    if (med < quick_medium_threshold) {
        quick <- work
    } else if (med < medium_slow_threshold) {
        medium <- work
    } else {
        slow <- work
    }
...
func main() {
    ...
    // endless loop
    log.Fatal(http.ListenAndServe(fmt.Sprintf(":%d", listen_port), nil))
